...
# edited by Spencer to deal with some new cases
...
parser.add_argument(
    ...
    type=int,
    default=2,
    help="(int: 2) Flag to determine if to swtich proline isomerization state. 0,1, or 2. 2: Won't edit prolines. 0: fs kaiB (cis to trans), 1: gs kaiB (trans to cis)",
)
parser.add_argument(
    "-i",
    "--iso_param",
...
parser.add_argument(
    ...
    "--coord",
    default="",
    help="(str: '') Name of the pdb file in the ./pdb folder. Omit the .pdb ending",
)
parser.add_argument(
    "-s",
    "--sim_id",
...
sp.check_output("source /project/dinner/scguo/upside2/sourceme.sh", shell=True)
## END ADAM EDIT ##
...
# write input command
with open (f"{input_dir}/in.txt", mode='w+') as f:
    arg_names = ['python', sys.argv[0]]
    for k, v in vars(args).items():
        arg_names.append(f'--{k}')
        arg_names.append(str(v))
    f.write(" ".join(arg_names))
...
if args.pro_flag == 0:
    ...
    print("Only switching P71 (trans to cis)...")
    sp.check_output("sed -i 's/PTLAKVLPPP/PTLAKVLP\*PP/g' " + fasta, shell=True)
else:
...
sbatch_opts = sbatch_opts.format(
    account, job_name, log_file, run_time, partition, args.n_rep, mem, qos
)

print("Running...")
cmd = 'sbatch {} --wrap="{}/obj/upside {} {}"'.format(
    sbatch_opts, upside_path, upside_opts, h5_files_str
)
