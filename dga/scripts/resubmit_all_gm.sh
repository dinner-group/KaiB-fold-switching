...
conda activate /beagle3/dinner/scguo/envs/md
source /project2/dinner/scguo/upside2-md/sourceme.sh
home_dir="/project/dinner/scguo/kaiB"
...
# each job in jobarray runs 3 starting points (since GM4 is slower)
start_coord_index=\$((1536 * $N + jobindex * 3))
...
for i in {0..2}; do
    for iso in cis trans ; do
        ...
        python \$run_script -p \$p -i \$iso_params_file -t 0.$temperature -c \$coordhead -s \$iso -o \$run_dir -m 200000
    done
done
EOT
