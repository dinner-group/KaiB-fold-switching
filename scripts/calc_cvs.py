...
def best_hummer_qtot(traj, ref, selection1, selection2):
    ...
    BETA_CONST = 50  # 1/nm
    LAMBDA_CONST = 1.5  # 1.5 for coarse-grained simulations
    NATIVE_CUTOFF = 0.45  # nanometers
...
def main():
    ...
    stride = 1
    backbone = "((name N or name CA or name C or name O) and (resid 0 to 93))"
    gs_file = "/project2/dinner/darrenjl/kaiB/03022023_restart_trans_gs/run_0/2qke_mutated.run.00.up"  #'/project/dinner/aanto/kaiB/up2/20220415_gs1_iso/full_tuned0/2qke_mon.run.00.up'
    ...
    gs_blue = gs_top.select("{} and (resid 87 to 93)".format(backbone))
    gs_selections = [gs_orange, gs_green, gs_blue]
    gs_specials = [None, None, gs_red]

    fs_file = "/project2/dinner/darrenjl/kaiB/02272023_cis_fs/run_0/fs_mutated.run.00.up"  #'/project/dinner/aanto/kaiB/up2/20220415_fs1_iso/full_tuned0/fs_wtseq.run.00.up'
    ...
    fs_blue = fs_top.select("{} and (resid 83 to 93)".format(backbone))
    fs_selections = [fs_orange, fs_green, fs_blue]
    fs_specials = [fs_red, fs_orange, None]

    red_selection = "(resid 8 to 13 or resid 20 to 34 or resid 40 to 45)"
    selection = "(resid 8 to 13 or resid 20 to 34 or resid 40 to 45 or resid 61 to 66 or resid 71 to 80 or resid 86 to 92)"
    fs2_selection = "(resid 8 to 13 or resid 20 to 34 or resid 40 to 45 or resid 63 to 68 or resid 72 to 76 or resid 83 to 93)"
    ...
    for i, dfile in enumerate(dfiles):
        ...
        # Featurize w/Pyemma to get pairwise distances
        # raw_feat = pyemma.coordinates.load(h5_file, features=feat)

        # Calculate RMSDS (2 different flavors) to the gs
        p_rmsd = md.rmsd(t, t_gs, atom_indices=refs1, ref_atom_indices=refs)
        r_rmsd = md.rmsd(t, t_gs, atom_indices=red_refs1, ref_atom_indices=red_refs)

        # Calculate RMSD to the fs
        f_rmsd = md.rmsd(t, t_fs, atom_indices=fs_refs1, ref_atom_indices=fs_refs)
        ...
        #     else:
        #         qs.append(best_hummer_q(t, t_fs, fs_selection, 50, 1.5))
        #     if gs_special is not None:
        #         qs.append(best_hummer_qtot(t, t_gs, gs_selection, gs_special))
        #     else:
        #         qs.append(best_hummer_q(t, t_gs, gs_selection, 50, 1.5))
        # qs = np.asarray(qs)
        # smoothed_qs = moving_average(qs.T, n=3)

        # Accumulate trajs and CVs
        # smoothed_fs_qtots.append(smoothed_qs.T)
        # fs_qtots.append(qs)
        f_rmsds.append(f_rmsd)
        p_rmsds.append(p_rmsd)
        r_rmsds.append(r_rmsd)
    ...
    pickle.dump(f_rmsds, open(f"{input_dir}/{output_dir}/{output_name}_{run_dir}_f_rmsds.pkl", "wb"))
    pickle.dump(p_rmsds, open(f"{input_dir}/{output_dir}/{output_name}_{run_dir}_p_rmsds.pkl", "wb"))
    pickle.dump(r_rmsds, open(f"{input_dir}/{output_dir}/{output_name}_{run_dir}_r_rmsds.pkl", "wb"))
    # pickle.dump(raw_feats, open(f"{input_dir}/{output_dir}/{output_name}_{run_dir}_raw_feats.pkl", "wb"))
    # pickle.dump(trajs, open(f"{input_dir}/{output_dir}/{output_name}_{run_dir}_trajs.pkl", "wb"))
